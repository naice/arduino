...
#define RELAIS_TOGGLE_DELAY     1000
#define SONAR_PROBE_DELAY       10
#define LED_PIN                 6
#define NUM_LEDS                36
...
#define PIR_PIN                 3
#define ANIMATION_DURATION      120000
#define LIFECYCLE_DURATION      300000

NewPing sonar(PIN_TRIG, PIN_ECHO, MAX_DIST); // NewPing
SonarSwitch sonarSwitch(MEASUREMENT_THRESHOLD, MIN_DISTANCE, MAX_DISTANCE, SONAR_PROBE_DELAY);
RelaisSwitch relaisSwitch(PIN_RELAIS);

CRGB leds[NUM_LEDS];
unsigned long relaisToggled = 0;
...
void sonarRelaisLoop() {
  unsigned int distanceCM = sonar.ping_cm();
  bool isSwitched = sonarSwitch.Switch(distanceCM);

  unsigned long elapsedMillisSinceRelaisToggled = millis() - relaisToggled;
  if (isSwitched && elapsedMillisSinceRelaisToggled > RELAIS_TOGGLE_DELAY) {
    relaisToggled = millis();
    relaisSwitch.Toggle();
    Serial.print(secs); Serial.print(": ");
    Serial.println("relais toggled");
  }
}
...
void pir_led_loop()
{
  ...
  {
    Serial.print(secs); Serial.print(": ");
    Serial.println("intterrupt by motion.");
